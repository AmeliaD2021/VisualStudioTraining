...
namespace ConsoleApp1
{
    class Program
    {
        ...
        /// <summary>
        /// Read people from file, convert to strong typed list.
        /// Use a method for a third party library, copy to this project and use to get distinct years
        /// </summary>
        private static void GetDistinctBirthYears()
        {
            ...
            if (File.Exists(_fileName))
            {
                ...
                List<int> years = people
                    .DistinctBy(person => person.BirthDate.Year)
                    .Select(person => person.BirthDate.Year).OrderBy(year => year)
                    .ToList();


                List<List<int>> chunked = years.ChunkBy(10);

                foreach (var list in chunked)
                {
